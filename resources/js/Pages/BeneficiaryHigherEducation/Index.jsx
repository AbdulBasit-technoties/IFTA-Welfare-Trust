  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        router.delete(route('beneficiarys-educations.destroy', id), {
  ...
  return (
    <AuthenticatedLayout auth={auth}>
      ...
      <div className="px-10 py-20">
        <div className="flex sm:flex-row flex-col gap-3 sm:gap-0 font-semibold sm:items-center leading-tight text-gray-800 sm:text-xl text-lg justify-between mb-4 md:px-4">
          <h2>Beneficiary Higher Education</h2>
          <div className="text-gray-800 md:text-sm text-xs">
            Per page {beneficiaries.total}/{beneficiaries.to || beneficiaries.length}
            <Link
              href={route('beneficiarys-educations.create')}
                ...
                <tr className="space-y-3 font-bold" key={item.id}>
                  ...
                  <td className="text-sm">
                    <div className="flex gap-2">
                      <Link href={route('beneficiarys-educations.edit', item.id)} className="bg-gray-800 p-3 rounded-full text-white">
                        <FaEdit />
                      </Link>
                      <Link href={route('beneficiarys-educations.show', item.id)} className="bg-gray-800 p-3 rounded-full text-white">
                        <FaEye />
                      </Link>
